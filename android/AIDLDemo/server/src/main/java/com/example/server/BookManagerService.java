...
public class BookManagerService extends Service {
    ...
    /**
     * 不用普通ArrayList的原因是，服务端可能被多个客户端同时绑定, aild方法就被多个binder线程同时执行, 因此
     * 要保证线程同步，而CopyOnWriteArrayList已经为我们实现了操作list时的线程同步， 这样调用aidl方法时就不
     * 要考虑线程同步的问题了.
     */
     ...
     * 用RemoteCallbackList，而不用ArrayList的原因是, 客户端的对象注册进来后， 服务端会通过它反序列化出一个
     * 新的对象保存一起，所以说已经不是同一个对象了. 在客户端调用解除注册方法时， 在list中根本就找不到它的对象，
     * 也就无法从list中删除客户端的对象. 而RemoteCallbackList的内部保存的是客户端对象底层的binder对象, 这个
     * binder对象在客户端对象和反序列化的新对象中是同一个对象,  RemoteCallbackList的实现原理就是利用的这个特性.
     */
    ...
    public void onCreate() {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                //在每休眠5秒后，会自动添加一本书， 并通过OnNewBookArrived()方法通知所有观察者。
    ...
    private void addNotify() {
        ...
        try {
            ...
            Log.d("onNewBookArrived", "registener listener size:" + N);
            for (int i = 0; i < N; i++) {
