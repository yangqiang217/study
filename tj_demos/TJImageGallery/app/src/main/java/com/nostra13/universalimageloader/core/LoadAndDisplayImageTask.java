	...
	private Bitmap tryLoadBitmap() throws TaskCancelledException {
		...
		try {
			...
			if (bitmap == null || bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
				...
				if (!options.isCacheOnDisk() && imageUriForDecoding != null && imageUriForDecoding.startsWith("file://")) {
				    String path = imageUriForDecoding.replaceFirst("file://", "");
//                    if (!TextUtils.isEmpty(path) && new File(path).exists()) {
//                        try {
//                            ImageEncryptTool.decrypt(path);
//                        } catch (IOException e) {
//                            e.printStackTrace();
//                        }
//                    }
